...
import { addDaysUTC, toYMD } from "../../utils/addDaysUTC";
...
export async function getHabitsWeekService({ userId, start, startDate }: WeekParams) {
...
export async function createHabitService(title: string, userId: number) {
  try {
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { id: true },
...
export async function editHabitService({ userId, habitId, title }: { userId: number; habitId: number; title: string }) {
  try {
    const habit = await prisma.habit.findFirst({
      where: { id: habitId, userId },
      select: { id: true },
...
export async function deleteHabitService({ userId, habitId }: { userId: number; habitId: number }) {
  try {
    const habit = await prisma.habit.findFirst({
      where: { id: habitId, userId },
      select: { id: true },
